...
| 0034 | The console-brain model is an operational input, never knowledge: pinned per project via committed `brain_model` in config.yaml, `PRAXIS_BRAIN_MODEL` env for CI, `--model` per run, precedence flag > env > config > the claude CLI default; verbatim pass-through (no model-name validation), no hardcoded default, no per-goal overrides; the pin is committed because a cheaper model can mis-navigate and false-alarm, so teammates and CI must regress with the same brain capability (Proposed) |
