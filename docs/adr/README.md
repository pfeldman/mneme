...
| 0032 | Whether an observed-but-still-streaming network request counts as a typed partial confirmation: premise falsified by the persisted regress observation event (the agent observed the streamed POST's 200 fine; matching, not observability, failed), the open PARTIAL/STREAMING question is moot once ADR-0033 removes the matching failure, teach guidance corrected to the narrow caveat (stream STATUS is observable; only still-open stream CONTENT belongs in visible-effect types) (Withdrawn) |
| 0033 | Enumerated signals are confirmed by identity with mandatory evidence: the envelope confirms each seed by ref with a separate evidence field, the body system-stamps the seed echo and still evaluates every structured check/predicate fail-closed, advisory grounding tripwires are recorded not gating, Jaccard is demoted to unsolicited observations, teach goes structured-first, ADR-0032 withdrawn (premise falsified: matching, not streaming observability, caused both live false REGRESSED) (Accepted) |
| 0034 | The console-brain model is an operational input, never knowledge: pinned per project via committed `brain_model` in config.yaml, `PRAXIS_BRAIN_MODEL` env for CI, `--model` per run, precedence flag > env > config > the claude CLI default; verbatim pass-through (no model-name validation), no hardcoded default, no per-goal overrides; the pin is committed because a cheaper model can mis-navigate and false-alarm, so teammates and CI must regress with the same brain capability (Accepted) |
| 0035 | Multi-environment deployments: product-level knowledge, per-environment evidence: one committed `environments` map (`--env` flag > `PRAXIS_ENV` > `default_env`) supplies the base_url to the agent prompt at run time so goal YAMLs stay deployment-agnostic; events gain an `environment` field and the projection partitions by it (an env is a partition key, NEVER a source dimension: cross-env corroboration, decay-anchor folding, and env-derived source_ids are forbidden); per-env secrets overlay (`.praxis.secrets.<env>`) and sessions (`PRAXIS_AUTH_STATE_<ENV>_<ROLE>`, `.praxis.auth/<env>/`, no cross-env fallback); runs/reports/JUnit tagged by env; an undeclared project is byte-identical in behavior; amends ADR-0021 (one repo per PRODUCT, environments are not tenants) (Proposed) |
