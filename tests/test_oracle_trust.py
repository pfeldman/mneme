...
def summary(type_, *, kind="success", present=(True,), seeded=False, ver="1",
    ...
    s.source_ids = {"spec" if seeded else "agent-1"}
    return s
...
def test_single_agent_type_is_not_believed() -> None:
    ...
    assert classify(s, oracle_diverse=False, agreeing={SignalType.BEHAVIORAL},
                    now=NOW, current_version="1") is Status.CONTESTED


def test_two_different_types_agreeing_are_believed() -> None:
    a = summary(SignalType.BEHAVIORAL)
    b = summary(SignalType.NETWORK)
    assert oracle_believed([a, b]) is True
    agreeing = agreeing_types([a, b])
    assert classify(a, oracle_diverse=True, agreeing=agreeing, now=NOW,
                    current_version="1") is Status.BELIEVED


def test_two_observations_of_same_type_do_not_create_diversity() -> None:
    a = summary(SignalType.NETWORK, present=(True, True))
    b = summary(SignalType.NETWORK, present=(True, True, True))
...
def test_seeded_oracle_is_believed_from_cold_start() -> None:
    ...
    assert classify(seed, oracle_diverse=True, agreeing={SignalType.BEHAVIORAL},
...
def test_flip_flop_is_quarantined() -> None:
    ...
    assert classify(flip, oracle_diverse=True, agreeing={SignalType.BEHAVIORAL},
...
def test_single_disagreement_is_contested_not_quarantined() -> None:
    ...
    assert classify(contra, oracle_diverse=True, agreeing=set(),
                    now=NOW, current_version="1") is Status.CONTESTED
...
def test_old_or_off_version_signal_is_stale() -> None:
    ...
    assert classify(old, oracle_diverse=True, agreeing={SignalType.BEHAVIORAL},
                    now=NOW, current_version="1") is Status.STALE
    offver = summary(SignalType.BEHAVIORAL, seeded=True, ver="0.9")
    assert classify(offver, oracle_diverse=True, agreeing={SignalType.BEHAVIORAL},
