...
Phase 2 (ADR-0014) adds `CandidateEvent` as a SIBLING type to `ObservationEvent`.
It carries agent-proposed risks or uncertainties (NOT signal observations); it has
its own `schema_version` and is read/written through dedicated store paths so the
diversity gate over signal observations cannot accidentally count candidate writes
as evidence (ADR-0008 source-independence hardening).
"""
